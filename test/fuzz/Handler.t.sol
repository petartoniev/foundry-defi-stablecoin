...
contract Handler is Test {
    ...
    function mintDsc(uint256 amount, uint256 addressSeed) public {
        ...
        // only an address that has deposited collateral can mint dsc.
        (uint256 totalDscMinted, uint256 collateralValueInUsd) = dsce.getAccountInformation(sender);
        console.log("Total DSC Minted: %d", totalDscMinted);
        console.log("Total Collateral: %d", collateralValueInUsd);
        int256 maxDscToMint = (int256(collateralValueInUsd) / 2) - int256(totalDscMinted);
        console.log("Max dsc to mint: %d", uint256(maxDscToMint));
        if (maxDscToMint <= 0) {
            return;
        }
